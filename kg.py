...
class KnowledgeGradient:
	def __init__(self, n_arms, T):
		...
		self.gamma = .81 
		self.thetas = np.array([self._expected_beta(self.prior_alpha, self.prior_beta) for _ in n_arms])
	...
	# def _estimate(self, q, thetas, successes, failures, k, r, priors, gamma):
		...
		if random.random() < 1 - gamma:
			thetas = np.array([self._expected_beta(self.prior_alpha, self.prior_beta) for _ in self.n_arms])

		pr_observation = np.ones((n_arms, 100))
